	...
	private String generateExternStringFromToken(InternToken internToken, boolean trimNumbers) {
		switch (internToken.getInternTokenType()) {
			...
			case DISTANCE_TO_TOUCH_POSITION:
				return CatroidApplication.getAppContext().getString(R.string
						.formula_editor_object_distance_to_touch_position);
			case DISTANCE_TO_SPRITE_POSITION:
				String distanceToSpritePosition = CatroidApplication.getAppContext().getString(R.string
						.formula_editor_object_distance_to);
				return distanceToSpritePosition + "(" + internToken.getTokenStringValue() + ")";
//				return internToken.getTokenStringValue();
